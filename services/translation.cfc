component accessors=true {
  ...
  public any function init( root, config, jsonService ) {
    ...
    variables.languageStruct = jsonService.deserialize( jsonFileContents );
    ...
    if ( !isNull( locale ) ) {
      variables.localeID = locale.getID( );
    }
  ...
  public string function cacheRead( required string translation, string localeID = getLocaleID(), boolean reload = false ) {
    ...
    lock scope="application" type="exclusive" timeout="5" {
      ...
      try {
        ...
        if ( structKeyExists( languageStruct, translation ) ) {
          translated = languageStruct[ translation ];
